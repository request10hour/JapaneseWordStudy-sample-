...
export default {
  ...
  created() {
    // console.log(this.extractHiragana('追い出された'));
    // console.log(this.makeRegExpWithExtractedHiragana(this.extractHiragana('追い出された')));
    // console.log(this.extractYomiganaByRegExp('追い出された', 'おいでされた'));
    this.runIfKanjiAndKanaIsDifferent(wordN5);
  },
  methods: {
    extractHiragana(str1) {
      ...
      for (let i = 0; i < str1.length; i++) {
        if (this.allHiraganaListWithDakuon.includes(str1[i])) {
          temp += str1[i];
        }
        else {
          if (temp != '') {
    ...
    extractYomiganaByRegExp(str1, str2) {
      let result = [];
      let extractedHiragana = this.extractHiragana(str1);
      ...
      wordList.forEach(word => {
        ...
        if (word.kanji != word.kana) {
          ...
          for (let i = 0; i < word.kanji.length; i++) {
            if (this.allHiraganaListWithDakuon.includes(word.kanji[i])) {
          ...
          if (isHiraganaIncluded) {
            ...
            console.log(this.extractYomiganaByRegExp(word.kanji, word.kana));
          }
        }
      });
    }
